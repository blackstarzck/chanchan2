...
1. 저장소의 디자인 handoff 데이터를 최신 Release 기준으로 갱신한다.
2. 필요하면 `npm run design:release -- <design-version>`로 frozen snapshot을 만든다.
3. `npm run tokens:sync`를 실행한다.
4. `packages/ui`에서 컴포넌트를 수정하거나 추가한다.
5. `apps/docs`에서 예제를 확인한다.
6. `npm run typecheck`를 실행한다.
7. `npm run build`를 실행한다.
8. 배포할 변경이 있다면 `npm run changeset`을 실행한다.
9. PR을 머지하고 GitHub Actions 릴리스를 확인한다.
...
- `design/handoff/current/*.json`은 사람이 관리하는 handoff 파일
- `packages/tokens/src/token-source.json`, `tokens.ts`, `theme.css`는 기계가 다시 만들어주는 파일

중요한 규칙:

- 생성 파일은 직접 수정하지 않는다
- 항상 `design/handoff/current`를 수정한 뒤 `npm run tokens:sync`를 실행한다
...
- 토큰은 항상 `design/handoff/current`만 수정한다
